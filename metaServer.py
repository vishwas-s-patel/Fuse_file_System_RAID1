...
# Return a corrupted value
  def corrupt(self, key):
    key1 = key.data
    val = self.data[key1]
    unpickled_val = pickle.loads(val[0])
    corrupt_val = unpickled_val + 'corrupt'
    corrupted_tuple = (pickle.dumps(corrupt_val), )
    val = corrupted_tuple + val[1:]
    self.data[key1] = val
    val1 = self.get(key)

    return val1
 
   # Terminate a data-server cleanly
  def terminate(self):
    pid = os.getpid()
    os.kill(pid, 9)
    return 0
 
   # Get list(keys) for values
  def list_contents(self):
    key_val = self.data.keys()
    return key_val
...
def main():
  ...
  start_server = Pool(len(argv) - 1)

  start_server.map(serve, server_ports)
